...
export default function Subscribers() {
  ...
  function handleSave(data) {
    setSubscribers((prev) => prev.some((s) => s.id === data.id)
  ...
  function applyTagToSelected() {
    ...
    setSubscribers((prev) =>
      prev.map((s) => selectedIds.includes(s.id) && !s.tags.includes(tagInput)
        ? { ...s, tags: [...s.tags, tagInput.trim()] } : s)
    );
    setTagInput("");
    setSelectedIds([]);
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {selectedIds.length > 0 && (
          <div className="flex items-center gap-2 bg-brand/5 border border-brand/20 rounded-md px-3 py-1.5">
